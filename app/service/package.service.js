...
const {
  ...
  seprateKeysValues,
  concateUpdateString,
  concateInsertKeysString,
  concateInsertValuesString,
} = require("../../utils");
// const moment = require('moment');
const select = "id, name, price";
...
exports.create = function (req) {
  ...
  return new Promise((resolve, reject) => {
    try {
      sql.query(
        `INSERT INTO ${table} (${strKeys}) VALUES (${strValues})`,
        [...fieldValues],
        (err, result) => {
          if (err) {
            reject(err);
...
exports.find = function () {
  return new Promise(function (resolve, reject) {
    try {
      sql.query(`SELECT ${select} FROM ${table}`, (err, result) => {
        if (err) {
          reject(err);
        } else {
          resolve(result);
